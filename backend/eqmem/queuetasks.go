...
// NewQueue creates a new task store, allowing retrieval, deletion,
...
func (s *Queue) Set(id uuid.UUID, val *entroq.Task) {
	defer un(lock(s))
...
func (s *Queue) Delete(id uuid.UUID) {
	defer un(lock(s))
...
func (s *Queue) Update(id uuid.UUID, f func(*entroq.Task) *entroq.Task) error {
	defer un(lock(s))
...
func (s *Queue) Get(id uuid.UUID) (*entroq.Task, bool) {
	// No need to lock.
	t, ok := s.byID.Load(id)
	if !ok {
		return nil, false
...
func (s *Queue) Range(f func(uuid.UUID, *entroq.Task) bool) {
	// No need to lock.
