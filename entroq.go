...
// claimant IDs when autogenerated. By default, this will be GenHex16.
...
//
// By default, it uses GenHex16 for task and claimant IDs and assigns
...
// ClaimOpt specifies task claim functionality.
type ClaimOpt func(*ClaimQuery)

// WithClaimant sets the claimant ID for a claim operation. When not set, uses
// the internal default for this client. There are only very specific cases
// where this might be needed, such as proxying over a network. Otherwise don't
// use it.
func WithClaimant(id string) ClaimOpt {
...
func (m *Modification) String() string {
	...
	if len(m.DocChanges) != 0 {
		out = append(out, fmt.Sprintf("doc-chg: %v", m.DocChanges))
	}
	if len(m.DocDeletes) != 0 {
		out = append(out, fmt.Sprintf("doc-del: %v", m.DocDeletes))
	}
...
// transaction can proceed. That checks versions appropriate for all different
...
func (m *Modification) DependencyError(found map[string]*Task, foundDocs map[string]*Doc) error {
	...
	if len(presentInserts) > 0 || len(missingChanges) > 0 || len(claimedChanges) > 0 || len(missingDeletes) > 0 || len(claimedDeletes) > 0 || len(missingDepends) > 0 ||
		len(presentDocIns) > 0 || len(missingDocChg) > 0 || len(claimedDocChg) > 0 || len(missingDocDel) > 0 || len(claimedDocDel) > 0 || len(missingDocDep) > 0 {
		return &DependencyError{
			...
			DocInserts: presentDocIns,
			DocChanges: missingDocChg,
			DocDeletes: missingDocDel,
			DocDepends: missingDocDep,
			DocClaims:  append(claimedDocDel, claimedDocChg...),
		}
	}
	return nil
}

// DocKey returns a namespace-qualified key for use in foundDocs maps,
...
func (m *DependencyError) Copy() *DependencyError {
	e := &DependencyError{
		...
		DocInserts: make([]*DocID, len(m.DocInserts)),
		DocDepends: make([]*DocID, len(m.DocDepends)),
		DocDeletes: make([]*DocID, len(m.DocDeletes)),
		DocChanges: make([]*DocID, len(m.DocChanges)),
	...
	copy(e.DocInserts, m.DocInserts)
	copy(e.DocDepends, m.DocDepends)
	copy(e.DocDeletes, m.DocDeletes)
	copy(e.DocChanges, m.DocChanges)
	copy(e.DocClaims, m.DocClaims)
	return e
}
...
// docs). Useful for backends to do "force" operations, making it easy to ignore
...
func (m *DependencyError) Error() string {
	...
	if len(m.DocDepends) > 0 {
		lines = append(lines, "\tmissing doc depends:")
		for _, did := range m.DocDepends {
			lines = append(lines, fmt.Sprintf("\t\t%s", did))
		}
	}
	if len(m.DocDeletes) > 0 {
		lines = append(lines, "\tmissing doc deletes:")
		for _, did := range m.DocDeletes {
			lines = append(lines, fmt.Sprintf("\t\t%s", did))
		}
	}
	if len(m.DocChanges) > 0 {
		lines = append(lines, "\tmissing doc changes:")
		for _, did := range m.DocChanges {
			lines = append(lines, fmt.Sprintf("\t\t%s", did))
		}
	}
