...
docker run --rm -it -v "$PWD":/src "$imgname" \
  bash -c "python -m grpc_tools.protoc -Iproto --python_out="$target_dir"/ --grpc_python_out="$target_dir"/ proto/*.proto && sed -i'' -e 's/^import .*_pb2 as/from . &/' '$target_dir/entroq_pb2_grpc.py'"
