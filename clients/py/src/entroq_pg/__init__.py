...
eqpg backend (which installs the stored procedures entroq_modify,
entroq_try_claim_one, etc.).
...
# ---------------------------------------------------------------------------
# JSONB encoding helpers for entroq_modify
# ---------------------------------------------------------------------------
        ...
        try:
            rows = self.conn.execute(
                '''SELECT kind, id, version, queue, at, created, modified,
                   ...
                   FROM entroq_modify(%s, %s::jsonb, %s::jsonb, %s::jsonb, %s::jsonb)''',
                (
                    claimant,
                    _encode_ids(depends),
                    _encode_ids(deletes),
...
class EntroQ:
    ...
    def time(self) -> datetime:
        ...
        with psycopg.connect(self._connstr, row_factory=dict_row) as conn:
    ...
    def queues(self, prefix='', exact=(), limit=0) -> list:
        ...
        with psycopg.connect(self._connstr, row_factory=dict_row) as conn:
            return conn.execute(
                'SELECT * FROM entroq_queues(%s, %s, %s)',
    ...
    def tasks(self, queue='', limit=0, omit_values=False) -> List[Task]:
        ...
        with psycopg.connect(self._connstr, row_factory=dict_row) as conn:
            return [_row_to_task(r) for r in conn.execute(
                'SELECT * FROM entroq_tasks(%s, %s, %s)',
    ...
    def try_claim(self, queue, duration_ms=30000) -> Optional[Task]:
        ...
        with psycopg.connect(self._connstr, row_factory=dict_row) as conn:
            rows = conn.execute(
                "SELECT * FROM entroq_try_claim(%s, %s, %s::interval)",
    ...
    def claim(self, queue, duration_ms=30000, poll_ms=5000, timeout_s=None,
        ...
        with psycopg.connect(self._connstr) as lconn:
